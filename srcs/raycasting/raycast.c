...
void draw_floor_and_ceiling(t_cub *cub)
{
    for (int y = 0; y < WIN_HEIGHT / 2; y++)
        for (int x = 0; x < WIN_WIDTH; x++)
            mlx_pixel_put(cub->init_ptr, cub->win_ptr, x, y, (cub->ceil.red << 16) | (cub->ceil.green << 8) | cub->ceil.blue);

    for (int y = WIN_HEIGHT / 2; y < WIN_HEIGHT; y++)
        for (int x = 0; x < WIN_WIDTH; x++)
            mlx_pixel_put(cub->init_ptr, cub->win_ptr, x, y, (cub->floor.red << 16) | (cub->floor.green << 8) | cub->floor.blue);
}

void perform_dda(t_cub *cub, int *map_x, int *map_y, double *side_dist_x, double *side_dist_y, double delta_dist_x, double delta_dist_y, int step_x, int step_y, int *side)
{
    int hit = 0;
    while (hit == 0)
    {
...
void raycast(t_cub *cub)
{
    ...
    for (int x = 0; x < WIN_WIDTH; x++)
    {
        ...
        int map_x = (int)cub->player.pos_x + (int)ray_dir_x * 0.5;
        int map_y = (int)cub->player.pos_y + (int)ray_dir_y * 0.5;
        ...
        perform_dda(cub, &map_x, &map_y, &side_dist_x, &side_dist_y, delta_dist_x, delta_dist_y, step_x, step_y, &side);

        double perp_wall_dist = (side == 0) ? (map_x - cub->player.pos_x + (1 - step_x) / 2) / ray_dir_x : (map_y - cub->player.pos_y + (1 - step_y) / 2) / ray_dir_y;
